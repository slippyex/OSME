...
/* external replayer code (68k assembler bin) */
#include "replay_data/sndh_ice.h"
#include "replay_data/jamcrackerpro.h"
#include "replay_data/fc13_14.h"
#ifdef ahx_test
#include "replay_data/ahx.h"
#endif
/* end */
...
api68_t * api68_init(api68_init_t * init)
{
  ...
  /* Alloc API struct. */
  api = (api68_t*)SC68alloc(sizeof(api68_t));
  if (!api) {
    goto error;
  }
...
// ====== (START) ================ HIER WIRD DER EXTERNE PLAYER GELADEN =========================

	// code sequenz lädt den benötigten player direkt aus der library ^^
#if AMIGA && ATARI
	if(!strcmp(m->replay, "sndh_ice")) {
		is = istream_mem_create(sndh_ice,sizeof(sndh_ice),1);
	} else if(!strcmp(m->replay, "JamCrackerPro")) {
		is = istream_mem_create(jamcrackerpro,sizeof(jamcrackerpro),1);
	} else if(!strcmp(m->replay, "fc13_14")) {
		is = istream_mem_create(fc13_14,sizeof(fc13_14),1);
#ifdef ahx_test
	} else if(!strcmp(m->replay, "ahx")) {
		is = istream_mem_create(ahx,sizeof(ahx),1);
#endif
	} else {
		return API68_MIX_ERROR;
    }
#endif
#if AMIGA && !ATARI
	if(!strcmp(m->replay, "JamCrackerPro")) {
		is = istream_mem_create(jamcrackerpro,sizeof(jamcrackerpro),1);
	} else if(!strcmp(m->replay, "fc13_14")) {
		is = istream_mem_create(fc13_14,sizeof(fc13_14),1);
#ifdef ahx_test
...
int api68_process(api68_t * api, void * buf16st, int n, int p)
{
...
int load_mydisk(api68_t * api, disk68_t * d)
{
  int track;
  if (!api || !d) {
    goto error;
...
{
  return load_mydisk(api, file68_load(is));
}

int api68_load_mem(api68_t * api, const void * buffer, int len)
{
  return load_mydisk(api,(int)file68_load_mem(buffer, (int)len));
}

//api68_disk_t api68_disk_load_mem(const void * buffer, int len)
//{
//  return (api68_disk_t) file68_load_mem(buffer, len);
//}
...
int api68_open(api68_t * api, api68_disk_t disk)
{
  ...
  return load_mydisk(api, (disk68_t*)disk);
}
		     ...
		     api68_disk_t disk)
{
  ...
  d = (disk68_t*) disk ? (disk68_t*)disk : (disk68_t*)api->disk;
  if (!d) {
    return -1;
  }
